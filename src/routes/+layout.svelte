<script lang="ts">
  ...
  function navIcon(name: string, size: number, active: boolean): string {
    const c = active ? "var(--accent)" : "var(--text2)";
    const s = size ?? 20;
    const base = `width="${s}" height="${s}" viewBox="0 0 24 24" fill="none" stroke="${c}" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"`;
    const icons: Record<string, string> = {
      dashboard: `<svg ${base}><rect x="3" y="3" width="7" height="7" rx="1.5"/><rect x="14" y="3" width="7" height="7" rx="1.5"/><rect x="14" y="14" width="7" height="7" rx="1.5"/><rect x="3" y="14" width="7" height="7" rx="1.5"/></svg>`,
      log: `<svg ${base}><path d="M14 2H6a2 2 0 00-2 2v16a2 2 0 002 2h12a2 2 0 002-2V8z"/><polyline points="14 2 14 8 20 8"/><line x1="8" y1="13" x2="16" y2="13"/><line x1="8" y1="17" x2="13" y2="17"/></svg>`,
      analytics: `<svg ${base}><polyline points="22 12 18 12 15 21 9 3 6 12 2 12"/></svg>`,
      advanced: `<svg ${base}><circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/><line x1="11" y1="8" x2="11" y2="14"/><line x1="8" y1="11" x2="14" y2="11"/></svg>`,
      calendar: `<svg ${base}><rect x="3" y="4" width="18" height="18" rx="2"/><line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="21" y2="10"/></svg>`,
...
<div class="flex h-dvh overflow-hidden" style="background: var(--bg)">
  <!-- ── Desktop sidebar ─────────────────────────────────────────────────── -->
  <aside
    class="hide-mobile flex-shrink-0 flex flex-col transition-all duration-300 {sidebar
      ? 'w-60'
      : 'w-[60px]'}"
  ...
  >
    <!-- Logo row -->
    <div
      class="flex items-center gap-3 px-4 h-[60px] flex-shrink-0"
    ...
    >
      ...
      {#if sidebar}
        <div class="flex-1 min-w-0">
          <div class="font-bold text-sm truncate" style="color: var(--text)">
            Trading Journal
          </div>
      ...
      >
        <svg
          ...
          stroke-width="2"
        >
          {#if sidebar}<path d="M15 18l-6-6 6-6" />{:else}<path
              d="M9 18l6-6-6-6"
            />{/if}
        </svg>
      </button>
        ...
        {#if currentAccount}
          <div class="flex justify-between items-center mt-1.5 px-0.5">
            <span class="text-xs" style="color: var(--text3)"
              >{currentAccount.broker}</span
            >
            <span
              class="text-xs font-semibold font-mono {currentAccount.current_balance >=
      ...
      {#each navItems as item}
        ...
        >
          <span class="flex-shrink-0 w-5 h-5 flex items-center justify-center">
            {@html navIcon(item.icon, 18, path === item.href)}
          </span>
  ...
  <div class="flex-1 flex flex-col overflow-hidden">
    <!-- Mobile top bar -->
    <header
      class="hide-desktop flex items-center justify-between px-4 h-14 flex-shrink-0"
    ...
    >
      ...
      <div class="flex items-center gap-2">
        ...
        {#if accountList.length > 0}
          <select
            class="text-xs rounded-lg px-2 py-1.5 font-medium"
    ...
    <!-- ── Mobile bottom nav ────────────────────────────────────────────── -->
    <nav
      class="hide-desktop flex-shrink-0 flex items-center"
    ...
    >
      {#each navItems as item}
        <a
          href={item.href}
          class="nav-link-bottom {path === item.href ? 'active' : ''}"
        >
          {@html navIcon(item.icon, 20, path === item.href)}
